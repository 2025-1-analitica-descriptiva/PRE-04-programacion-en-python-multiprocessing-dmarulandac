...
def load_input(input_directory):
    ...
    def make_iterator_from_multiple_files(input_directory):
        input_directory = os.path.join(input_directory, "*") #lista de archivos del directorio
        files = glob.glob(input_directory)
        with fileinput.input(files=files) as file:#lee secuencialmente un archivo
            yield from file #itera una linea a la vez y espera

    if os.path.isfile(input_directory):
        return make_iterator_from_single_file(input_directory)

    return make_iterator_from_multiple_files(input_directory) #iterador qque recorre secuencialmente
#una linea a la vez
...
# Preprocesamiento
#
def preprocessing(x): #recibe la linea, la convierte en miniscula, borra puntuacion, la borra y linea procesada
...
def line_preprocessing(sequence):
    ...
    with Pool() as pool:
        return pool.map(preprocessing, sequence) #pasa los registros en paralelo a los
    #Procesadores
...
#
def map_line(x):
    return [(w, 1) for w in x.split()] #lista por cada linea fisica del archivo 
#que contiene (palabra,1)
...
def reducer(sequence):
    ...
    def merge_results(chunks):
        ...
        return list(final_result.items()) #sumariza todo

    num_chunks = os.cpu_count() #cantidad de cpus 
    chunks = chunkify(sequence, num_chunks) #lista partida en bloques
